...
<div class="cart-list-wrapper">
  <template v-if="cartGoodsList.length!=0">
    ...
    <div class="goods-info">
      ...
      <p class="goods-money">￥{{item.gprice}}</p>
      <div class="btn-group">
        <div class="change">
          <button class="decrease" @click="reduceGoods(item)">-</button>
          <input type="number" v-model.number="item.gnum" @blur="handleBlur(item)">
...
<script type="text/javascript">
  ...
  export default {
    ...
    created() {
      this.axios.get('http://localhost:3000/carts').then((res) => {
    ...
    methods: {
      ...
      handleBlur(goods) {
        /*计算购物车的数量*/
        let num = goods.gnum == '' ? 1 : goods.gnum
