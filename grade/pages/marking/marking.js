...
Page({
  ...
  selectStudentId(e) {
    ...
    var symbolMark = item.symbolMark;
    symbolMark = symbolMark ? symbolMark : "";
    this.setData({
      swiper,
      ...
      if (res.code == 10000) {
        ...
        for (let j = 0; j < picdata.length; j++) {
          ...
          if (picdata[j].answerPostion.length > 0) {
            ...
            var w = parseInt(picdata[j].answerPostion[0].width);
            var h = parseInt(picdata[j].answerPostion[0].height);
            var x = parseInt(picdata[j].answerPostion[0].positionX);
            var y = parseInt(picdata[j].answerPostion[0].positionY);
  ...
  forwardHandle(e){
    ...
    var write = this.data.write;//是否划线
    var text = this.data.text;//是否备注
    if (!write){
      if(forward == 1){//前进
